...
export class MdxdocClient {
  ...
  docs = {
    ...
    export: (documentId: string, format = "mdx") => this.request<SourceResponse & { format: string }>(`/documents/${documentId}/export?format=${encodeURIComponent(format)}`),
    ...
    createSuggestion: (documentId: string, body: { type: string; anchor: unknown; before?: unknown; after?: unknown; baseVersion: number; changesetId?: string }) => this.request(`/documents/${documentId}/suggestions`, { method: "POST", body }),
